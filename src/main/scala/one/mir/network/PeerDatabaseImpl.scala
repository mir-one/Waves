...
class PeerDatabaseImpl(settings: NetworkSettings) extends PeerDatabase with ScorexLogging {
  ...
  for (a <- settings.knownPeers.view.map(inetSocketAddress(_, 1402))) {
  ...
  for (f <- settings.file if f.exists()) try {
    JsonFileStorage.load[PeersPersistenceType](f.getCanonicalPath).foreach(a => touch(inetSocketAddress(a, 1402)))
