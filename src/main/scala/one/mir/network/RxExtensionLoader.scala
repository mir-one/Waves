...
import one.mir.network.RxExtensionLoader.ApplierState.Buffer
import one.mir.network.RxExtensionLoader.LoaderState.WithPeer
import one.mir.network.RxScoreObserver.{ChannelClosedAndSyncWith, SyncWith}
...
object RxExtensionLoader extends ScorexLogging {
  ...
  def apply(syncTimeOut: FiniteDuration,
            ...
            timeoutSubject: Subject[Channel, Channel])(extensionApplier: (Channel, ExtensionBlocks) => Task[ApplyExtensionResult])
