...
object FunctionalitySettings {
  ...
  val configPath = "mir.blockchain.custom.functionality"
}
...
object GenesisSettings {
  val MAINNET = GenesisSettings(
    1460678400000L,
    1465742577614L,
    Constants.UnitsInWave * Constants.TotalMir,
    ByteStr.decodeBase58("FSH8eAAzZNqnG8xgTZtz5xuLqXySsXgAjmFEC25hXMbEufiGjqWPnGCZFt6gLiVLJny16ipxRNAkkzjjhqTjBE2").toOption,
    List(
      GenesisTransactionSettings("3PAWwWa6GbwcJaFzwqXQN5KQm7H96Y7SHTQ", Constants.UnitsInWave * Constants.TotalMir - 5 * Constants.UnitsInWave),
  ...
  val TESTNET = GenesisSettings(
    ...
    List(
      GenesisTransactionSettings("3My3KZgFQ3CrVHgz6vGRt8687sH4oAA1qp8", (Constants.UnitsInWave * Constants.TotalMir * 0.04).toLong),
      GenesisTransactionSettings("3NBVqYXrapgJP9atQccdBPAgJPwHDKkh6A8", (Constants.UnitsInWave * Constants.TotalMir * 0.02).toLong),
      GenesisTransactionSettings("3N5GRqzDBhjVXnCn44baHcz2GoZy5qLxtTh", (Constants.UnitsInWave * Constants.TotalMir * 0.02).toLong),
      GenesisTransactionSettings("3NCBMxgdghg4tUhEEffSXy11L6hUi6fcBpd", (Constants.UnitsInWave * Constants.TotalMir * 0.02).toLong),
      GenesisTransactionSettings("3N18z4B8kyyQ96PhN5eyhCAbg4j49CgwZJx",
                                 (Constants.UnitsInWave * Constants.TotalMir - Constants.UnitsInWave * Constants.TotalMir * 0.1).toLong)
    ),
    153722867L,
    60.seconds
  )
...
object BlockchainSettings {
  val configPath: String = "mir.blockchain"
  ...
  def fromConfig(config: Config): BlockchainSettings = {
    ...
    val (addressSchemeCharacter, functionalitySettings, genesisSettings) = blockchainType match {
      ...
      case BlockchainType.CUSTOM =>
        ...
        val functionalitySettings  = config.as[FunctionalitySettings]("mir.blockchain.custom.functionality")
        val genesisSettings        = config.as[GenesisSettings]("mir.blockchain.custom.genesis")
