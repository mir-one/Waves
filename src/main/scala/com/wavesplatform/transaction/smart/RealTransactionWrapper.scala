...
import one.mir.lang.v1.traits.domain.Tx.{Header, Proven}
    ...
    tx match {
      ...
      case ms: MassTransferTransaction =>
        Tx.MassTransfer(
          ...
          transfers = ms.transfers.map(r => one.mir.lang.v1.traits.domain.Tx.TransferItem(r.address, r.amount)).toIndexedSeq,
