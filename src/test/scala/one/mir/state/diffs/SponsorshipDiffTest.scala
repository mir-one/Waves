...
import one.mir.settings.{Constants, TestFunctionalitySettings}
...
import one.mir.transaction.assets.{IssueTransactionV1, SponsorFeeTransaction}
...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      ...
      mirOverspend = TransferTransactionV1
        .selfSigned(None, master, recipient.toAddress, 1000000, ts + 3, Some(assetId), fee, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, assetOverspend, insufficientFee, mirOverspend)

    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, mirOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(mirOverspend)), s) { blockDiffEi =>
          if (mirOverspend.fee > issue.quantity)
            blockDiffEi should produce("unavailable funds")
          else
  ...
  property("not enough mir to pay fee after leasing") {
    val s = settings(0)
    val setup = for {
      master <- accountGen
      alice  <- accountGen
  ...
  property("sponsor has no MIR but receives them just in time") {
    ...
    val setup = for {
      ...
      assetTransfer = TransferTransactionV1
        ...
        .right
        .get
      mirTransfer = TransferTransactionV1
        .selfSigned(None, master, recipient, 99800000, ts + 4, None, 100000, Array.emptyByteArray)
        .right
        .get
      backMirTransfer = TransferTransactionV1
        .selfSigned(None, recipient, master, 100000, ts + 5, Some(assetId), 100, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issue, sponsor, assetTransfer, mirTransfer, backMirTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, mirTransfer, backMirTransfer) =>
        assertDiffAndState(Seq(block(Seq(genesis, issue, sponsor, assetTransfer, mirTransfer))), block(Seq(backMirTransfer)), s) {
          case (diff, state) =>
