...
import one.mir.lang.ScriptVersion.Versions.V2
import one.mir.lang.Testing.evaluated
    ...
    val ctx =
      PureContext
        ...
        CryptoContext
          .build(Global) |+|
        MirContext
          .build(V2, new MirEnvironment(chainId, Coeval(null), null, EmptyBlockchain), isTokenContext = true)
    ...
    val ctx =
      PureContext.build(V2) |+|
        CryptoContext
          .build(Global) |+|
        MirContext
          .build(V2, new MirEnvironment(chainId, Coeval(t), null, EmptyBlockchain), isTokenContext = false)
