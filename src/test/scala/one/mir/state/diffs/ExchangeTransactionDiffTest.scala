...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("preserves mir invariant, stores match info, rewards matcher") {
  ...
  property("buy mir without enough money for fee") {
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createMirTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createMirTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
