...
class PeerDatabaseImplSpecification extends path.FreeSpecLike with Matchers {
  ...
  private val config1   = ConfigFactory.parseString("""mir.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 2s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings1 = config1.as[NetworkSettings]("mir.network")

  private val config2   = ConfigFactory.parseString("""mir.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 10s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings2 = config2.as[NetworkSettings]("mir.network")
  ...
  "Peer database" - {
    ...
    "filters out wildcard addresses" in {
      database.addCandidate(new InetSocketAddress("0.0.0.0", 1402))
      database.randomPeer(Set(address1, address2)) shouldBe None
    }

    "if blacklisting is disabled" - {
      "should clear blacklist at start" in {
        val databaseFile = Files.createTempFile("mir-tests", "PeerDatabaseImplSpecification-blacklisting-clear").toAbsolutePath.toString
        val path         = if (File.separatorChar == '\\') databaseFile.replace('\\', '/') else databaseFile
        val prevConfig   = ConfigFactory.parseString(s"""mir.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val prevSettings = prevConfig.as[NetworkSettings]("mir.network")
        val prevDatabase = new PeerDatabaseImpl(prevSettings)
        prevDatabase.blacklist(address1, "I don't like it")
        prevDatabase.close()

        val config   = ConfigFactory.parseString(s"""mir.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |  enable-blacklisting = no
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val settings = config.as[NetworkSettings]("mir.network")
