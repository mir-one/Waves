...
import one.mir.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
import one.mir.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
import one.mir.transaction.transfer.TransferTransaction.MaxAttachmentSize
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.mir balance is decreased by fee.") {
  ...
  test("mir mass transfer changes mir balances") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough mir") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative mir balance")
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.mir), Transfer(secondAddress, 2.mir), Transfer(thirdAddress, 3.mir))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.mir)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.mir)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.buildWithCurrentNetworkByte(alias).explicitGet().stringRepr, 2.mir)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
