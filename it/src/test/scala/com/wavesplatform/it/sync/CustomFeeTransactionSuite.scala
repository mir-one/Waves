...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
      | mir.fees.transfer.$assetId = 100000
      | mir.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("mir.miner.enable=no").withFallback(minerConfig)
