...
import one.mir.state.{AssetDistribution, AssetDistributionPage, DataEntry, EitherExt2, Portfolio}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/mir/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateMir/$height").as[Map[String, Long]]
